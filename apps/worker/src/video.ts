...
/** Edge of the average-hash grid used for loop comparison. 16×16 = 256 cells. */
const LOOP_HASH_EDGE = 16;

/**
 * Read one frame as a {@link LOOP_HASH_EDGE}² greyscale bitmap.
 *
 * Raw bytes rather than PNG: the comparison wants pixels, and asking ffmpeg for
 * `rawvideo` avoids decoding a PNG in-process for a 256-byte answer. `-ss`
 * before `-i` seeks rather than decoding to the timestamp, which matters when
 * the second sample is at the end of a long file.
 *
 * Returns null when the frame cannot be read, so the caller reports the
 * measurement as absent rather than substituting a number.
 */
async function frameHash(filePath: string, atSeconds: number): Promise<Uint8Array | null> {
  const cells = LOOP_HASH_EDGE * LOOP_HASH_EDGE;
  try {
    const { stdout } = await execFileAsync(
      ...
      [
        ...
        '-i', filePath,
        '-frames:v', '1',
        '-vf', `scale=${LOOP_HASH_EDGE}:${LOOP_HASH_EDGE},format=gray`,
        '-f', 'rawvideo',
        '-',
      ],
    ...
    return bytes.length === cells ? new Uint8Array(bytes) : null;
  } catch {
    return null;
  }
}
...
/**
 * How much the last frame looks like the first, 0..1. 1 is identical.
 *
 * Feeds `retention.not_loop_ready`, which had been unmeasured on every render
 * ever produced: `RetentionProbe.loopSimilarity` is optional and nothing wrote
 * it, so the rule that TikTok and Reels care most about has never run. Same
 * shape of defect as §74's `frameDelta` and the `frameLuminance` stdout bug —
 * an optional probe field with no writer reads exactly like a check that passes.
 *
 * **Why a coarse spatial grid and not the scalar signals.** §73 and §74 both
 * turn on the fact that a *whole-frame average* cannot see Halyard's content: a
 * light card with a small region of changing dark text barely moves the mean.
 * That argument does not carry over here, because 256 cells are not an average
 * — text moving between two thirds of the frame changes the cells it occupies
 * even when the overall mean is flat. And a loop reads or fails to read at
 * roughly this resolution: the question is whether the ending looks like the
 * opening at a glance, which is what a viewer registers before deciding to
 * replay.
 *
 * The last frame is sampled slightly inside the end. Seeking exactly to
 * `duration` lands past the final frame on most containers and returns nothing.
 */
export async function measureLoopSimilarity(
  filePath: string,
  durationSeconds: number,
): Promise<number | null> {
  ...
  const [first, last] = await Promise.all([
    frameHash(filePath, 0),
    frameHash(filePath, Math.max(0, durationSeconds - 0.15)),
  ]);
  if (!first || !last || first.length !== last.length || first.length === 0) return null;

  let total = 0;
  for (let i = 0; i < first.length; i += 1) total += Math.abs(first[i]! - last[i]!);
  const similarity = 1 - total / (first.length * 255);

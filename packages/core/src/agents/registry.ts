...
export const AGENT_REGISTRY: AgentContract[] = [
  ...
  {
    agentId: 'stock-footage-finder',
    name: 'Stock Footage Finder',
    team: 'content',
    kind: 'deterministic',
    version: '1.0',
    purpose:
      'Turns a screenplay scene grounded as `footage` into a licensed portrait clip of its subject (§478). The screenwriter perceives which lines are about something happening; this searches, rotates against clips the product has used, downloads, and stores. Code decides what a clip may carry: illustration only, never evidence, never the product.',
    model: null,
    runtimeAttribution: 'explicit',
    promptVersions: [],
    implementation: 'apps/worker/src/beatFootage.ts#footageForBeats',
    inputSchema: { beats: 'Array<{ text, subject, seconds }>', productName: 'string', pieceSubject: 'string | undefined' },
    outputSchema: { assetId: 'string | null', clipId: 'string | null', seconds: 'number | null', reason: 'string | null' },
    tools: ['pexels:videos/search'],
    expectedCallers: ['apps/worker/src/handlers/generate.ts'],
    downstreamConsumer: 'render.ts stages the clip into the bundle; the Narrative ground plays it under the same camera grammar as a still',

...
export const AGENT_REGISTRY: AgentContract[] = [
  ...
  },
  {
    ...
    implementation: 'packages/core/src/generation/prompts.ts#buildIdeaGeneratorPrompt',
    inputSchema: { productBrief: 'string', mixState: 'MixState', recentIdeas: 'string[]' },
    ...
    observations: [],
    acceptanceTests: [],
    declaredStatus: 'implemented_no_caller',
    statusNote:
      'Only the prompt builder exists. There is no agent function that sends it, and nothing calls the builder — not even a test. Ideas are currently scored but never generated by a model.',
  },
  ...
  // ── Team: learning ───────────────────────────────────────────────────────
  {
    ...
    tools: ['llm'],
    expectedCallers: [],
    downstreamConsumer: 'rejection_clusters — intended for brand_voices.dont_rules',
    permissions: ['write:rejection_clusters'],
    retries: 0,
    timeoutMs: null,
    state: ['rejection_clusters', 'brand_voices'],
    observations: ['what the operator consistently rejects'],
    acceptanceTests: ['packages/core/src/generation/hooks.test.ts'],
    declaredStatus: 'implemented_no_caller',
    statusNote:
      'Referenced only by its own tests. The per-item loop is wired — rejecting appends to brand_voices.anti_examples — but nothing groups those into patterns. Needs a body of rejections, which arrives once the queue is worked.',
  },
  ...
  // ── Team: content (media) ────────────────────────────────────────────────
  {
    ...
    declaredStatus: 'blocked',
    statusNote:
      'There is no long-form footage in the system to clip from, so there is nothing for a caller to pass it.',
  },
];
